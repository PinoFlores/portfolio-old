...
import { useDynamicStyled } from "../../hooks";
...
export const Text = withTheme(
  ({ children, variant = "body2", theme, ...props }: TextProps) => {
    const styled = useDynamicStyled({ ...props, theme });

    const Typography = _.get(TextVariants, variant, TextVariants.body1);

    const color = _.get(
      theme?.palette,
      _.get(props, "color", "text.primary"),
